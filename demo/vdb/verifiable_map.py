...
class Node:
    ...
    def debug_dump(self, seq):
        tab = '  ' * self._depth
        print(tab + "H: " + b64encode(self.hash(seq)).decode())
        v = self._value.get(seq)
        if v is None:
            x = self.left(seq)
            if x:
                ...
                print(tab + "L: " + b64encode(self.left_hash(seq)).decode())
            x = self.right(seq)
            if x:
                print(tab + "R:")
                x.debug_dump(seq)
            else:
                print(tab + "R: " + b64encode(self.right_hash(seq)).decode())
        else:
            print(tab + "V:" + repr(v))
            print(tab + "P:" + ''.join(
                [str(int(x)) for x in self.path(seq)]))


# Take a key as string and produce 256 boolean values indicating left (False) or right (True)
def construct_key_path(key):
    g = ''.join(('%8s' % bin(x)[2:]) for x in hashit(key))
    return [x == '1' for x in g [:SHA_LEN]]
...
def compress_proof(proof):
    counter = 0
    rv = []
    for y in ['' if x == DEFAULTS[idx + 1] else b64encode(x) for idx, x
              in enumerate(proof)]:
        if y == '':
            counter += 1
        else:
...
def decompress_proof(proof):
    ...
    for y in proof:
        if type(y) == int:
            ...
            pp.append(y)
    return [DEFAULTS[idx + 1] if x == '' else b64decode(x) for idx, x in
            enumerate(pp)]
...
class VerifiableMap:
    ...
    def get(self, key, seq):
        ...
        if len(proof) < SHA_LEN:
            ...
            if v is not None:
                p = last.path(seq)
                while len(proof) < SHA_LEN and path[len(proof)] == p[
                    len(proof)]:
    ...
    def get_tree_head(self, seq=None):
        ...
        rv = {
            'tree_size': seq,
            'sha256_root_hash': b64encode(self._root.hash(seq)),
        }
        return rv
...
def recalc_tree_hash(key, value, proof):
    t = hashleaf(value)
    chs = construct_key_path(key)[::-1]
    ps = decompress_proof(proof)[::-1]
    for ch, p in zip(chs, ps):
        if ch:
            t = hashparent(p, t)
        else:
            t = hashparent(t, p)
