...
from verifiable_map import recalc_tree_hash
...
class VerifiableDatabase(VerifiableBase):
    ...
    def set(self, key, value):
        self._log.append(
            json.dumps({'operation': 'set', 'key': key, 'value': value}))
...
def test(db, query, tree_size, exp_val):
    ...
    assert val == exp_val
    assert recalc_tree_hash(query, str(val) if val else '', proof) == \
           db.get_tree_head(tree_size)['sha256_root_hash']
...
class ReplCmd(cmd.Cmd):
    ...
    def help_sth(self):
        print('sth <integer> - Updates tree to sequence number and print STH. '
    ...
    def help_set(self):
        print('set <key> <value> - set key (string) '
              'to the specified value (string)')
    ...
    def do_get(self, arg):
        ...
        print('Log hash:  ',
              self.db.get_tree_head(v)['log_tree_head']['sha256_root_hash'])
        print('Tree size: ', self.db.get_tree_head(v)['tree_size'])

    def help_get(self):
        print(
            'get <key> <integer> - get value as of this sequence number.  Leave blank for latest.')
    ...
    def help_dump(self):
        print(
            'dump <integer> - dump the tree as of this sequence number.  Leave blank for latest.')

    def do_log(self, arg=''):
        for i, x in enumerate(self.db.get_log_entries(0,
                                                      self.db.get_tree_head()[
                                                          'tree_size'] - 1)):
            print(i, x)
